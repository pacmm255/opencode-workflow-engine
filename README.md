...
The supervisor drafts acceptance criteria, independently checks their fidelity to your objective, checks whether they are already met, then creates the next useful workflow for unmet requirements. Each workflow is followed by fresh independent verification. It continues until acceptance succeeds, you pause/stop it, or a prerequisite needs your input. It never treats a finished workflow alone as a finished goal. Referenced implementation plans are pinned as immutable input snapshots: workers implement the code/tests, not rewrite the plan to narrow the goal. Explicitly edit the goal to accept changed requirements.

There is **no goal-wide token, cost, workflow-count, iteration, or duration ceiling**. Usage is informational. Individual workflows still have configured finite timeouts, agent/concurrency limits, cancellation, and normal OpenCode permissions. Goal children request per-response limits where supported (8,192 tokens for coordination; 16,384 for implementation, or a lower host/model limit). Custom/OAuth transports keep their own wire parameters; independent local guards bound observed response text and reported tokens, excluding tool-result content. These checks are not an exact pre-spend cap when a provider reports usage only after completion. Starting a goal does not authorize publishing, destructive changes, or expanding its scope. Repeated recoverable failures back off without an aggregate retry cap; provider quota/rate-limit reset times persist across restarts without launching replacement workers before the reset. Denied authority, unsupported provider parameters, and output-limit failures need review rather than repeated identical paid attempts. Repeatedly invalid acceptance contracts require configuration review rather than freezing invented requirements.

Goal mode complements Ultracode: the goal determines **what must be achieved and when to stop**; Ultracode supplies workflow behavior and supported reasoning effort. Enable `/ultracode`, then start `/workflow-goal`. Goal-owned results go to one supervisor, without extra Ultracode continuation prompts. You can also use goals without enabling Ultracode. Coordinator stages inherit the effective default/session model and reasoning variant regardless of pool ordering; worker plans choose from the actual pool. Unavailable configured models wait for repair rather than silently switching providers. The dashboard displays the coordinator model, waiting reason, and retry time; the parent receives progress, distinct failure/wait, and terminal updates without a second execution loop.
...
State and paginated history live in a separate SQLite database under OpenCode's workflow application-data directory, outside your repository. Reserved operation/run IDs and a project lease prevent duplicate scheduling across plugin instances. Active goals reconcile known children after restart; supervisor shutdown is distinct from a user pause, and paused/cancelled goals stay stopped. OpenCode must be running with the project loaded—this is not a separate always-on daemon. Native controls show an offline supervisor when its lease is absent. Each project admits one goal operation at a time; pause active goals before independent workflows or parent edits. Use separate sessions for other goal/loop plugins: their internal state is not automatically detected or disabled, and workflow-goal children cannot invoke their goal APIs.
