export const commands = {
  ...
  workflows: { description: "List workflow runs and progress", template: "Use workflow_runs with action list to show this session's runs. $ARGUMENTS" },
  "workflow-config": { description: "Choose workflow models and limits", template: "Use workflow_config show to list exact connected provider/model IDs and current settings. Help the user choose allowed models with the question tool if needed, then use workflow_config set. User request: $ARGUMENTS" },
