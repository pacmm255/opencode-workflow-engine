...
export function goalInput(goal: GoalState, context: RunContext): WorkflowInput {
  ...
  const instruction = goal.stage === "defining"
    ? `WORKFLOW_GOAL_CONTRACT: Derive a concise acceptance checklist covering ALL explicit outcomes and constraints in the objective and pinned specification, including completion/quality requirements. This child only drafts the checklist; implementation happens in later children. NEVER turn this child's read-only procedure into an acceptance requirement or a restriction on implementation. Implementing a plan requires source/test changes, not a plan-file-only scope. Do not invent merge targets, scope restrictions, or requirements for already having a successful implementation before work starts. Do not write constraints into the plan. Ambiguous external authority may need a concrete user question, but missing implementation and failing tests are remaining work. Return criteria and summary.`
  ...
  const data = drafting ? JSON.stringify({ objective: goal.objective, specificationInputs: specificationInputs(goal), proposedChecklist: goal.draftContract, correction: goal.reason }) : contractData(goal);
